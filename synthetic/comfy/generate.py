...
class ComfyClient:
    ...
    def queue_prompt(self, workflow: Dict[str, Any], client_id: str = "defect_batch") -> str:
        ...
        if not r.ok:
            print(f"[ERROR] /prompt returned {r.status_code}: {r.text}", file=sys.stderr)
            r.raise_for_status()
        js = r.json()
...
def patch_workflow(
    ...
    *,
    ...
    try:
        wf2[saver_node_id]["inputs"]["filename_prefix"] = f"{dslug}/{dslug}_syn"
    except KeyError as e:
        raise KeyError(f"Missing saver node {saver_node_id} inputs.filename_prefix. Error: {e}") from e
